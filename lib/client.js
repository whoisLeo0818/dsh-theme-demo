...
window.__ModuleLoader__.load({
  id: '@dsh-external/dsh-plugin-steel-skin',
  factory: () => {
    var module = { exports: {} }
    ...
    // Edit this block to retune the skin; nothing below it needs to change.
    // ---------------------------------------------------------------------
    const SKIN = {
      ...
      // null for a plain dark background.
      image: '/steel-skin/assets/background.png',
    ...
    function apply(ctx) {
      ...
      ctx.effect(() => ctx.theme.overrideTokens(
        '@dsh-external/dsh-plugin-steel-skin',
        buildTokens(),
      ), 'steel-skin: alias token layer')
      ...
      ctx.effect(() => {
        ...
        tag.dataset.plugin = '@dsh-external/dsh-plugin-steel-skin'
        tag.textContent = buildCss()
        document.head.appendChild(tag)
        return () => { tag.remove() }
      }, 'steel-skin: component stylesheet')
    }
