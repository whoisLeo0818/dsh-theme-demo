...
**[PolyForm Noncommercial License 1.0.0](https://polyformproject.org/licenses/noncommercial/1.0.0) —— 仅供演示，不得商用。**

允许：个人学习、研究、实验、业余项目，以及慈善机构、教育机构、公共研究组织、公共安全/医疗组织、环保组织、政府机构的使用。

不允许：任何商业用途。

选标准许可而不是自己写一句「不得商用」，是因为它有 SPDX 标识符（`PolyForm-Noncommercial-1.0.0`），措辞经过起草、边界清晰，工具链也能识别。自己拟的条款容易在「什么算商用」上留下歧义。

完整条款见 [`LICENSE`](LICENSE)。

### `assets/` 里的图片不在许可范围内

**那两张背景图是第三方版权内容，仅作演示占位。** 本项目的许可只覆盖代码。

如果你要长期使用或者把这个项目分享出去，请替换成你自己有权使用的图片 —— 丢进 `assets/`，改 `SKIN.image` 即可（记得保持接近 16:9）。
